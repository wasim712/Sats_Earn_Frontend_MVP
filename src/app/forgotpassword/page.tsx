...
export default function ForgetPasswordPage() {
  ...
  // --- UI RENDERING ONLY BELOW THIS POINT ---
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-[440px] relative z-10">
        ...
        <div className="bg-[#0a0a0a]/90 backdrop-blur-2xl pt-8 pb-10 px-6 shadow-[0_20px_60px_rgba(0,0,0,0.4)] border border-[#1a1a1a] sm:rounded-[32px] sm:px-10 overflow-hidden relative">
          
          {/* Card Top Glow */}
          <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-sats-orange-500/30 to-transparent" />

          {/* PREMIER LOGO HEADER (Inside Card, Linked to Home) */}
          <Link href="/" className="flex justify-center items-center mb-8 group w-fit mx-auto outline-none">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-[#050505] rounded-xl border border-sats-orange-500/30 flex items-center justify-center relative shadow-[0_0_15px_rgba(249,115,22,0.15)] group-hover:shadow-[0_0_25px_rgba(249,115,22,0.3)] group-hover:scale-105 transition-all duration-300">
                <Image width={40} height={40} className="rounded-xl object-cover" alt="SatsEarn Logo" src="/icon.png" />
              </div>
              <LogoText className="text-2xl font-extrabold tracking-tight text-white group-hover:opacity-90 transition-opacity" />
            </div>
          </Link>
          ...
          {step === 'request' && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              {errorMsg && (
                <div className="mb-6 bg-red-500/10 border border-red-500/20 text-red-400 p-3.5 rounded-xl text-sm font-semibold flex items-start gap-2.5 shadow-sm">
              ...
              <form onSubmit={handleRequestOtp} className="space-y-6">
                <div>
                  <label htmlFor="email" className="block text-[12px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Email Address</label>
          ...
          {step === 'reset' && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="mb-6 text-center">
                ...
                <p className="mt-2 text-sm text-gray-400 font-medium">Code sent to <span className="text-sats-orange-400 font-bold">{email}</span></p>
              </div>
              ...
              <form onSubmit={handleResetPassword} className="space-y-6">
                ...
                <div className="bg-[#050505] border border-[#1a1a1a] rounded-2xl p-5 relative overflow-hidden">
                  ...
                  <label className="flex items-center justify-center gap-2 text-[12px] font-black text-gray-400 uppercase tracking-widest mb-3">
                  ...
                  <input
                    ...
                    className="block w-full bg-[#111] border border-[#2a2a2a] text-white rounded-xl py-4 px-4 focus:ring-1 focus:ring-sats-orange-500 focus:border-sats-orange-500 transition-colors placeholder:text-gray-700 outline-none tracking-[0.5em] text-center font-black text-2xl uppercase shadow-inner"
                    placeholder="••••••"
                  />
                </div>
                ...
                <div className="space-y-5">
                  <div>
                    <label className="block text-[12px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">New Password</label>
                    <div className="relative group">
                    ...
                    <div className="mt-3 bg-[#050505] border border-[#1a1a1a] rounded-xl p-3 grid grid-cols-2 gap-y-2 gap-x-2">
                  ...
                  <div>
                    <label className="block text-[12px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Confirm Password</label>
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-sats-orange-500 transition-colors" />
                      <input
                        type={showPassword ? "text" : "password"}
                ...
                <button type="submit" disabled={isLoading || otp.length !== 6 || !isPasswordValid || newPassword !== confirmPassword} className={`${btnClasses} mt-2`}>
          ...
          {step === 'success' && (
            <div className="animate-in fade-in zoom-in-95 duration-500 text-center py-6">
              ...
              <p className="text-gray-400 font-medium text-sm mb-8 leading-relaxed">
                Your account is now secure. You can log in with your new password to continue earning.
              </p>
...
const inputClasses = "block w-full bg-[#111] border border-[#2a2a2a] text-white rounded-xl py-3.5 pl-12 pr-12 focus:ring-1 focus:ring-sats-orange-500 focus:border-sats-orange-500 transition-colors sm:text-sm placeholder:text-gray-600 outline-none font-medium shadow-inner";
...
function ValidationRule({ isValid, text }: { isValid: boolean, text: string }) {
  return (
    <div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 ${isValid ? 'text-green-500' : 'text-gray-600'}`}>
      <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${isValid ? 'opacity-100' : 'opacity-30'}`} />
