...
// Bitcoin coin: orange disc + â‚¿ glyph. r = radius.
...
function BannerPreview({ size, styleId, box = 250, copy = TAGLINES[0], scene = "none", mark = "icon", markPos = "top" }) {
  ...
  const Coin = ({ x, y, r, a }) => (
    <div style={{ position: "absolute", left: x - r, top: y - r, width: r * 2, height: r * 2, borderRadius: "50%", background: "linear-gradient(180deg,#ffb347,#f7931a)", opacity: a, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.9)", fontWeight: 900, fontSize: r * 1.3, pointerEvents: "none" }}>â‚¿</div>
  );
  let sceneEl = null;
  if (scene === "blobs") {
    sceneEl = (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 14 }}>
        ...
        <div style={resCard}>
          ...
          <div style={{ textAlign: "center", fontSize: 11, color: C.txt2, lineHeight: 1.6 }}>
            Display: <b style={{ color: C.txt }}>Satoshi</b> Â· Mono: <b style={{ color: C.txt }}>Space Mono</b>
          </div>
        </div>
      </div>
    </Section>
...
function AmbassadorProgram() {
  ...
  const submit = async () => {
    ...
    if (!user) {
      setFormStatus({
        type: 'error',
        message: 'You must be logged in to send an ambassador application.',
      });
      return;
    }

    if (!SHARE_EARN_EMAILJS_SERVICE_ID || !SHARE_EARN_EMAILJS_TEMPLATE_ID || !SHARE_EARN_EMAILJS_PUBLIC_KEY) {
      setFormStatus({
        type: 'error',
        message: 'Email service is not configured yet. Please add the Share & Earn EmailJS IDs first.',
      });
      return;
    }
    ...
    try {
      await emailjs.send(
        SHARE_EARN_EMAILJS_SERVICE_ID,
        SHARE_EARN_EMAILJS_TEMPLATE_ID,
        {
          from_name: form.name,
          reply_to: form.email,
          name: form.name,
        ...
        {
          publicKey: SHARE_EARN_EMAILJS_PUBLIC_KEY,
        }
      );
      setSent(true);
      setFormStatus({ type: 'success', message: "Application sent successfully. We'll review and get back to you." });
      setForm({ name: "", email: "", platform: "YouTube", profile: "", audience: "1k-10k", promo: "" });
    } catch (error) {
      console.error("Failed to send application:", error);
      setFormStatus({ type: 'error', message: 'Failed to send application. Please try again later.' });
    } finally {
      setSending(false);
    }
  };
    ...
    >
      ...
      <div style={{ background: C.surf2, border: `1px solid ${C.line2}`, borderRadius: 14, padding: 22 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 16, flexWrap: "wrap" }}>
          <button onClick={submit} disabled={!valid || sending} style={{ ...primaryBtn, opacity: valid && !sending ? 1 : 0.5, cursor: valid && !sending ? "pointer" : "not-allowed" }}>
            {sending ? 'Sending...' : 'Submit application'}
          </button>
          {sent && <span style={{ color: C.grn, fontSize: 13, fontWeight: 600 }}>✓ Request sent — we'll review and get back to you.</span>}
          {formStatus.message && !sent && (
            <span style={{ color: formStatus.type === 'error' ? '#f87171' : C.grn, fontSize: 13, fontWeight: 600 }}>
              {formStatus.message}
            </span>
          )}
          {!valid && <span style={{ color: C.faint, fontSize: 12 }}>Name, a valid email and your profile link are required.</span>}
        </div>
      </div>
    </Section>
  );
  ...
  return (
    <footer style={{ background: C.bg, borderTop: `1px solid ${C.line}`, padding: "60px 20px 32px", position: "relative", zIndex: 1, marginTop: 40 }}>
      ...
      <div className="se-footer-bottom" style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, paddingTop: 24, borderTop: `1px solid ${C.line}`, fontSize: 12, color: C.txt2, fontWeight: 700 }}>
        <span>Â© 2026 SatsEarn. All rights reserved.</span>
...
export default function SatsEarnShare() {
  ...
  const { data } = useAppSelector((state) => state.userReferrals);
  ...
  return (
    <div style={{ background: "transparent", minHeight: "100vh", color: C.txt, fontFamily: "'Satoshi', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", boxSizing: "border-box" }}>
      ...
      <div style={{ maxWidth: 1020, margin: "0 auto", padding: "40px 18px 40px" }}>
        {/* Hero */}
