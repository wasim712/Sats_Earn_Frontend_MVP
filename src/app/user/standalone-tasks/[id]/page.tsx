...
export default function StandaloneTaskDetailsPage() {
  ...
  const [selectedFiles, setSelectedFiles] = useState<{ [id: string]: File | null }>({});
  const [textInputs, setTextInputs] = useState<{ [id: string]: string }>({});
  const [isSubmitting, setIsSubmitting] = useState<{ [id: string]: boolean }>({});
  const [submissionResults, setSubmissionResults] = useState<{ [id: string]: TaskResult | null }>({});
  const [taskStatuses, setTaskStatuses] = useState<{ [id: string]: TaskStatus }>({});
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        ...
        setTask(data);
        const latestStatus = data.userSubmission?.status || data.submissions?.[0]?.status;
        setTaskStatuses({ [data.id]: mapSubmissionStatusToTaskStatus(latestStatus) });

      } catch (err: unknown) {
        setPageError(err instanceof Error ? err.message : 'Failed to load standalone task.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmitProof = async (id: string, proofType: string) => {
    ...
    try {
      ...
      if (proofType === 'SCREENSHOT') {
        ...
        formData.append('proofText', text.trim());
      ...
      const data = await parseObfuscatedJson<{ error?: string; status?: string; message?: string; submission?: { status?: string } }>(response);

      if (!response.ok) throw new Error(data.error || data.message || 'Submission failed.');

      setSubmissionResults((prev) => ({
        ...prev,
        [id]: { success: true, message: data.message || 'Submitted successfully!' },
      }));
      
      const mappedStatus = mapSubmissionStatusToTaskStatus(data?.submission?.status || 'pending_review');
      ...
      setSubmissionResults((prev) => ({
        ...prev,
        [id]: { success: false, message: err instanceof Error ? err.message : 'Submission failed.' },
      }));
    } finally {
  ...
  const currentStatus = task ? taskStatuses[task.id] ?? 'idle' : 'idle';
  const allDone = currentStatus === 'completed' || currentStatus === 'pending_review';
  const completedCount = allDone ? 1 : 0;
  const totalTasks = 1;
  const progressPct = allDone ? 100 : 0;
  ...
  const reqPlat = String(task?.requiredPlatform || task?.campaign?.requiredPlatform || 'NONE').toUpperCase();
    ...
    return (
      <div className="max-w-md mx-auto mt-20 px-4 text-center">
        ...
        <p className="text-sm text-white/35 mb-8 leading-relaxed">
          {pageError || "We couldn't load this standalone task. It may have ended or been removed."}
        </p>
        <button
          onClick={() => router.back()}
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start flex-1 min-h-0">
          ...
          <div className="flex-1 min-w-0 w-full space-y-6 lg:overflow-y-auto lg:h-full lg:pr-3 pb-24 lg:pb-6 scrollbar-hide">
            ...
            <div className={`relative h-[140px] md:h-[180px] w-full overflow-hidden rounded-[24px] border ${isPremiumOnly ? 'border-violet-400/12 bg-[linear-gradient(180deg,#11111a_0%,#0e0a14_100%)]' : 'border-[#1a1a1a] bg-[#080808]'}`}>
              {/* Blurred background */}
              <Image src={coverImg} alt={task.title} fill className="object-cover blur-[80px] opacity-35" unoptimized />
              ...
              <div className="absolute inset-0 flex items-center justify-center p-6">
                <div className="relative aspect-square h-[60%] w-[60%] max-h-[80px] max-w-[80px]">
                  <Image src={coverImg} alt={task.title} fill className={`object-contain ${themeColors.primaryDropShadow}`} unoptimized />
                </div>
              </div>
            </div>
          ...
          <div className="w-full lg:w-[320px] shrink-0 space-y-4 lg:h-full lg:overflow-y-auto pb-24 lg:pb-6 scrollbar-hide">
              ...
              <div className="space-y-3 text-sm">
                ...
                {xpReward > 0 && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-violet-400">{xpReward.toLocaleString()} XP</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Campaign Info */}
            <div className="bg-[#080808] border border-[#1a1a1a] rounded-2xl p-5">
              <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.15em] mb-4">Task Info</h3>
